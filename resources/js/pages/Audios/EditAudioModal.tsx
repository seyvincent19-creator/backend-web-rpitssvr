...
interface Audio {
    ...
    duration: string; url: string; image: string;
}
interface Props { show: boolean; audio: Audio; onClose: () => void }
...
export default function EditAudioModal({ show, audio, onClose }: Props) {
    const fileRef = useRef<HTMLInputElement>(null);
    const [coverBroken, setCoverBroken] = useState(false);
    const { data, setData, setError, clearErrors, processing, errors } = useForm({
        title: audio.title, author: audio.author, year: audio.year, publisher: audio.publisher,
        type: audio.type, category: audio.category, language: audio.language,
        location: audio.location, duration: audio.duration, url: audio.url, image: null as File | null,
    });
    ...
    useEffect(() => {
        setData({
            title: audio.title, author: audio.author, year: audio.year, publisher: audio.publisher,
            type: audio.type, category: audio.category, language: audio.language,
            location: audio.location, duration: audio.duration, url: audio.url, image: null,
        });
    ...
    function submit(e: React.FormEvent) {
        ...
        router.post(`/audios/${audio.id}`, fd, {
            forceFormData: true,
            onSuccess: () => onClose(),
            onError: (errs) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h2 className="text-lg font-bold text-gray-900">Edit Audio</h2>
                ...
                <form onSubmit={submit} className="p-6 space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" value={data.type} onChange={e => setData('type', e.target.value)} className={inp} />
                    ...
                    <div>
                        ...
                        <div className="flex items-start gap-4">
                            {data.image ? (
                                ...
                                />
                            ) : audio.image && !coverBroken ? (
                                <img
                                    src={storageUrl(audio.image)}
                                    alt="Current cover"
                                    className="w-16 h-20 rounded-lg border object-cover flex-shrink-0"
                            ...
                            <div onClick={() => fileRef.current?.click()}
                                ...
                                <p className="text-xs text-gray-500">
                                    {audio.image && !coverBroken ? 'Click to change cover' : 'Click to upload cover'}
                                </p>

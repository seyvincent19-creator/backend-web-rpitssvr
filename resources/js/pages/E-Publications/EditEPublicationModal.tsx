...
interface EPublication {
    ...
    language: string; pages: number; location: string; url: string; image: string;
}
interface Props { show: boolean; epublication: EPublication; onClose: () => void }
...
export default function EditEPublicationModal({ show, epublication, onClose }: Props) {
    const fileRef = useRef<HTMLInputElement>(null);
    const [coverBroken, setCoverBroken] = useState(false);
    const { data, setData, setError, clearErrors, processing, errors } = useForm({
        title: epublication.title, author: epublication.author, year: epublication.year,
        publisher: epublication.publisher, language: epublication.language, pages: epublication.pages,
        location: epublication.location, url: epublication.url, image: null as File | null,
    });
    ...
    useEffect(() => {
        setData({
            title: epublication.title, author: epublication.author, year: epublication.year,
            publisher: epublication.publisher, language: epublication.language, pages: epublication.pages,
            location: epublication.location, url: epublication.url, image: null,
        });
    ...
    function submit(e: React.FormEvent) {
        ...
        fd.append('location', data.location); fd.append('url', data.url);
        if (data.image) fd.append('image', data.image);
        router.post(`/e-publications/${epublication.id}`, fd, {
            forceFormData: true,
            onSuccess: () => onClose(),
            onError: (errs) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}>
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    <h2 className="text-lg font-bold text-gray-900">Edit E-Publication</h2>
                ...
                <form onSubmit={submit} className="p-6 space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" value={data.location} onChange={e => setData('location', e.target.value)} className={inp} />
                    ...
                    <div>
                        ...
                        <div className="flex items-start gap-4">
                            {data.image ? (
                                ...
                                />
                            ) : epublication.image && !coverBroken ? (
                                <img
                                    src={storageUrl(epublication.image)}
                                    alt="Current cover"
                                    className="w-16 h-20 rounded-lg border object-cover flex-shrink-0"
                            ...
                            <div onClick={() => fileRef.current?.click()}
                                ...
                                <p className="text-xs text-gray-500">
                                    {epublication.image && !coverBroken ? 'Click to change cover' : 'Click to upload cover'}
                                </p>
